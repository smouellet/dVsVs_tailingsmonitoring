...
locs=pd.read_csv('station.txt') # select             
nsta=len(locs)
...
#FOR ACAUSAL t_ini=-3.5 seconds
t_ini = -3.5                                         # Time to start computing the dv/v (in second). Zero is at the zero lag time
...
for ifile in range(nfile):
    ...
    if ifile == win2-1:
        with open('dvv-arr-acausal-%s-%s.pickle' % (source,receiver), 'wb') as f:
            pickle.dump(dv_arr,f)
        f.close()
        with open('cc-arr-acausal-%s-%s.pickle' % (source,receiver), 'wb') as f:
            pickle.dump(cc_arr,f)
        f.close()
        with open('error-arr-acausal-%s-%s.pickle' % (source,receiver), 'wb') as f:
            pickle.dump(error_arr,f)
        f.close()
        dv_avg[isrc,:]=np.nanmean(dv_arr[win1:win2-1,:],axis=0, dtype=np.float64) 

        with open('dvv-avg-acausal-%s-%s.pickle' % (source,receiver), 'wb') as f:
